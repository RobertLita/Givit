...
def get_allocation(db: Session, allocation_id: int) -> RewardAllocation:
    return db.query(RewardAllocation).filter(RewardAllocation.id == allocation_id).first()
...
def create_allocation(db: Session, allocation: RewardAllocationCreate) -> RewardAllocation:
    db_allocation = RewardAllocation(userId=allocation.userId, rewardId=allocation.rewardId)
...
def delete_allocation(db: Session, allocation_id: int) -> RewardAllocation:
    db_allocation = db.query(RewardAllocation).filter(RewardAllocation.id == allocation_id).first()
    db.delete(db_allocation)
    db.commit()
    return db_allocation


def update_allocation(db: Session, existing_allocation: RewardAllocation, allocation_body: RewardAllocation) -> RewardAllocation:
