...
async def read_objects(skip: int = 0, limit: int = 10, db: Session = Depends(deps.get_db)):
...
async def read_object(object_id: int, db: Session = Depends(deps.get_db)):
    ...
    if db_object is None:
        raise HTTPException(status_code=404, detail=f'Object with id = {object_id} was not found')
    return db_object
...
async def get_organization(object_id: int, db: Session = Depends(deps.get_db)):
    ...
    return {"username": db_object.organization.username, "rating": db_object.organization.rating}


@router.patch("/{object_id}", response_model=Object)
async def update_object(object_id: int, object: Object, db: Session = Depends(deps.get_db)):
    db_object = crud_objects.get_object(db, object_id)
    if db_object is None:
        raise HTTPException(status_code=404, detail=f"Object with id = {object_id} was not found")
